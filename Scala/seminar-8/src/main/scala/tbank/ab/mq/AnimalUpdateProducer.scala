...
object AnimalUpdateProducer {
  ...
  private class Impl(topic: String, kafkaProducer: KafkaProducer[IO, AnimalId, AnimalId]) extends AnimalUpdateProducer[IO] {
  ...
  given Serializer[IO, AnimalId] =
    Serializer.instance[IO, AnimalId] { (_, _, animalId) =>
      IO.pure(animalId.toString.getBytes("UTF-8"))
    }
